...
class ProbabilityCalculator:
    ...
    def monte_carlo_numpy(self, sim_count: int, progress_callback=None) -> Dict[int, Dict[str, float]]:
        ...
        # 确保身份数量匹配
        total_needed = remaining_wolves + remaining_gods + remaining_humans + known_marks
        if total_needed != n_unknown:
            # 自动调整平民数量
            remaining_humans = n_unknown - remaining_wolves - remaining_gods - known_marks
            remaining_humans = max(0, remaining_humans)  # 确保不为负数
        ...
        for i in range(sim_count):
            total = remaining_wolves + gods_matrix[i] + humans_matrix[i]
            if total != n_unknown:
                # 自动修正
                humans_matrix[i] = n_unknown - remaining_wolves - gods_matrix[i]
        ...
        for batch in range(n_batches):
            ...
            for i in range(current_batch):
                ...
                roles.extend(['神民'] * int(gods_matrix[start + i]))
                roles.extend(['平民'] * int(humans_matrix[start + i]))

                # 确保长度正确
                if len(roles) != n_unknown:
                    diff = n_unknown - len(roles)
                    if diff > 0:
                        roles.extend(['平民'] * diff)
                    elif diff < 0:
                        roles = roles[:n_unknown]

                np.random.shuffle(roles)

                # 分配
                player_indices = list(range(n_unknown))
    ...
    def triangle_law_simulation(self, sim_count: int, progress_callback=None) -> Dict[int, Dict[str, float]]:
        ...
        # 验证剩余身份总数
        total_remaining = remaining_wolves + remaining_gods + remaining_humans + known_marks
        if total_remaining != n_unknown:
            # 自动调整
            diff = n_unknown - (remaining_wolves + remaining_gods + remaining_humans + known_marks)
            if diff > 0:
                remaining_humans += diff
            elif diff < 0:
                remaining_humans = max(0, remaining_humans + diff)
        ...
        for batch in range(n_batches):
            ...
            for i in range(current_batch):
                ...
                # 验证长度并调整
                if len(identity_pool) != n_unknown:
                    diff = n_unknown - len(identity_pool)
                    if diff > 0:
                        identity_pool.extend(['平民'] * diff)
                    elif diff < 0:
                        identity_pool = identity_pool[:n_unknown]
        ...
        results = {}
        for idx, player in enumerate(unknown_players):
            total = wolf_counts[idx] + god_counts[idx] + human_counts[idx]
            results[player] = {
                '狼人': wolf_counts[idx] / sim_count,
...
class WerewolfProbabilityApp:
    ...
    def create_toolbar(self):
        ...
        ttk.Label(toolbar, text="v1.1", font=("微软雅黑", 8)).pack(side=tk.RIGHT, padx=10)
    ...
    def create_speech_tab(self, parent):
        ...
        # 添加录音控制区域
        record_frame = ttk.LabelFrame(main_frame, text="🎤 语音转文字", padding="5")
        record_frame.pack(fill=tk.X, pady=5)

        record_control = ttk.Frame(record_frame)
        record_control.pack(fill=tk.X, pady=2)

        self.record_btn = ttk.Button(record_control, text="⏺ 开始录音",
                                     command=self.start_recording,
                                     style='Accent.TButton')
        self.record_btn.pack(side=tk.LEFT, padx=5)

        self.stop_btn = ttk.Button(record_control, text="⏹ 停止录音",
                                   command=self.stop_recording,
                                   state=tk.DISABLED)
        self.stop_btn.pack(side=tk.LEFT, padx=5)

        self.transcribe_btn = ttk.Button(record_control, text="📝 转写录音",
                                         command=self.transcribe_recording,
                                         state=tk.DISABLED)
        self.transcribe_btn.pack(side=tk.LEFT, padx=5)

        self.record_status = ttk.Label(record_frame, text="就绪",
                                       foreground=self.theme_manager.colors['gold'])
        ...
        # 初始化录音模块
        self.init_speech_recorder()

    def init_speech_recorder(self):
        """初始化语音转文字模块"""
        try:
            from speech_to_text import SpeechRecorder
            # 创建录音保存目录
            if not os.path.exists("audio_records"):
                os.makedirs("audio_records")

            self.speech_recorder = SpeechRecorder(save_dir="audio_records")
            self.recording_file = None
            self.log("✅ 语音转文字模块初始化成功")
        except ImportError as e:
            self.log(f"⚠️ 语音转文字模块未安装: {e}")
            self.record_btn.config(state=tk.DISABLED)
            self.record_status.config(text="模块未安装")
        except Exception as e:
            self.log(f"⚠️ 语音模块初始化失败: {e}")

    def start_recording(self):
        """开始录音"""
        try:
            # 生成文件名：玩家_轮次_时间戳.wav
            player = self.speech_player_var.get() or "all"
            round_text = self.speech_round_var.get() or "未知轮次"
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"audio_records/玩家{player}_{round_text}_{timestamp}.wav"

            self.speech_recorder.start_recording(filename)
            ...
            self.record_status.config(text="⏺ 录音中...", foreground="red")

            self.log(f"开始录音: {filename}")
        except Exception as e:
    ...
    def stop_recording(self):
        ...
        try:
            ...
            self.record_status.config(text="录音已停止，可转写", foreground=self.theme_manager.colors['gold'])

            self.log(f"录音已停止: {self.recording_file}")
        except Exception as e:
    ...
    def transcribe_recording(self):
        ...
        if not self.recording_file or not os.path.exists(self.recording_file):
            messagebox.showwarning("警告", "没有可转写的录音文件")
            return

        # 创建进度窗口
        progress = ProgressDialog(self.root, title="语音转文字", message="正在转写，请稍候...")

        try:
            # 执行转写
            result = self.speech_recorder.transcribe(self.recording_file)
            ...
            if result['success']:
                ...
                if current_text:
                    self.speech_text.insert(tk.END, f"\n\n[语音转写 {datetime.datetime.now().strftime('%H:%M:%S')}]\n")
                self.speech_text.insert(tk.END, result['text'])

                # 显示识别详情
                details = f"识别完成 | 时长: {result['duration']:.1f}秒 | 置信度: {result.get('confidence', 0):.1%}"
                self.record_status.config(text=details, foreground="green")

                self.log(f"语音转写完成: {result['text'][:50]}...")
            else:
                messagebox.showerror("转写失败", result.get('error', '未知错误'))
                self.record_status.config(text="转写失败", foreground="red")

        except Exception as e:
            progress.close()
            messagebox.showerror("错误", f"转写过程中出错: {e}")
            self.log(f"转写出错: {e}")
        finally:
            # 重置转写按钮状态
            self.transcribe_btn.config(state=tk.NORMAL)
...
MIT License © 2026 海斯
        ...
        # GitHub链接高亮
        start_pos = about_text.find("https://github.com/haisi-ai/Werewolves")
        if start_pos != -1:
            line_count = len(about_text[:start_pos].split('\n'))
            char_count = len(about_text[:start_pos].split('\n')[-1])
            start_idx = f"{line_count}.{char_count}"
            end_idx = f"{line_count}.{char_count + len('https://github.com/haisi-ai/Werewolves')}"
            ...
            text_widget.tag_bind("hyperlink", "<Button-1>",
                                 lambda e: webbrowser.open("https://github.com/haisi-ai/Werewolves"))
